...
public class mpu6050 : MonoBehaviour
{
    ...
    public GameObject target2;// is the gameobject to 
    ...
    float curr_angle_x2 = 90;
    float curr_angle_y = 0;
    float curr_angle_y2 = 90;
    float curr_angle_z = 0;

    float curr_offset_x = 3.48f;
    float curr_offset_y = 36.9f;
    float curr_offset_z = -9;
    ...
    void Update()
    {
        ...
        if (!dataString.Equals("NOT OPEN"))
        {
            ...
            if (enableTranslation) target1.transform.position = new Vector3(curr_offset_x, curr_offset_y, curr_offset_z);
            if (enableRotation) target1.transform.rotation = Quaternion.Euler(curr_angle_x2 * factor, curr_angle_y2 * factor ,- curr_angle_z * factor );

            if (enableTranslation) target2.transform.position = new Vector3(curr_offset_x1, curr_offset_y1, curr_offset_z1);
            if (enableRotation) target2.transform.rotation = Quaternion.Euler(curr_angle_x * factor, curr_angle_y * factor ,- curr_angle_z * factor );
        }
    }
}
